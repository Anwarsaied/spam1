...
client.on('ready', async () => {
  ...
  setInterval(() => {
client.channels.get("675001150910496791").send(`${ReBeL[Math.floor(Math.random() *ReBeL.length)]}`);//لا تنسى تحط أيدي الروم
},7000);
});
...
client.on('ready', async () => {
  ...
  setInterval(() => {
client.channels.get("675302447849275392").send(`${ReBeL[Math.floor(Math.random() *ReBeL.length)]}`);//لا تنسى تحط أيدي الروم
},7000);
});
